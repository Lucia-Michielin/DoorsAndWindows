...
summarise3 <- FullMeasuresForRoomType %>% 
  group_by(Type, RoomComparison) %>% 
  summarize(SurfaceM = mean(Surface), RatioM=mean(Ratio), count=n())

summarise3 <-arrange(summarise3, Type, desc(SurfaceM))
 
 
aggregate( Surface~Type,FullMeasuresForRoomType, mean )
